...
int treeHeight(Node* root){    //μεσω αναδρομης υπολογιζουμε το  υψος του δεντρου
    if(root==NULL){
        return 0;
    }
    int leftHeight=treeHeight(root->left);     //υπολογίζουμε το ύψος του αριστερού υποδέντρου
    int rightHeight=treeHeight(root->right);   //υπολογίζουμε το ύψος του δεξιού υποδέντρου
    if(rightHeight>leftHeight){             //αν το δεξι υπόδεντρο ειναι μεγαλύτερο(π.χ. φυλλο) τότε το ύψος του τρέχοντος κόμβου είναι το ύψος του δεξιού υποδέντρου + 1 (για τον ίδιο τον κόμβο)
        return rightHeight+1;       
    }else{
        return leftHeight+1;            // αν οχι, τύπωσε το ύψος του αριστερού υποδέντρου + 1 (για τον ίδιο τον κόμβο)
    }
}

void nodeDepths(Node* root,int depth){     //μεσω αναδρομης υπολογιζουμε το βαθος καθε κομβου
    head=tail=0;    
    enqueue(root);   
    while(head<tail){           //ελέγχουμε κάθε κόμβο που βγαίνει από την ουρά για να υπολογίσουμε το βάθος του και να προσθέσουμε τα παιδιά του στην ουρά με το βάθος τους
        int size=tail-head;          //διαχωρίζουμε τα επιπεδα του δεντρου ώστε να έχουμε σωστό βάθος για κάθε κόμβο, δηλαδή όταν βγαίνουν από την ουρά τα παιδιά του ίδιου επιπέδου, αυξάνουμε το βάθος κατά 1
        for(int i=0;i<size;i++){     //το for loop ελεγχει όλους τους κόμβους του τρέχοντος επιπέδου και τυπώνει για καθένα το depth που ορίζει το size,εντάσσοντας τα παιδιά τους στην ουρά για το επόμενο επίπεδο
            Node* temp=dequeue();       
            printf("Node's depth with data %d is %d\n",temp->data,depth);
            if(temp->left!=NULL){
                enqueue(temp->left);
            }
            if(temp->right!=NULL){
...
void freeTree(Node* root){     //μεσω αναδρομης απελευθερώνουμε τη μνήμη που καταλαμβάνει το δέντρο
    if(root!=NULL){
